...
final class MacHelper: @unchecked Sendable {
    ...
    private lazy var router: JsonRpcRouter = {
        ...
        router.register("cuelive.captureScreen") { [weak self] params in
            guard let self else { throw JsonRpcDispatchError.internalError("Helper is shutting down") }
            let object = (params as? [String: Any]) ?? [:]
        ...
        router.register("cuelive.pointAt") { [weak self] params in
            guard let self else { throw JsonRpcDispatchError.internalError("Helper is shutting down") }
            guard let object = params as? [String: Any] else {
                throw JsonRpcDispatchError.invalidParams("cuelive.pointAt requires params")
            }
            return self.ensureCueLive().pointAt(params: object)
        }
        router.register("cuelive.setVoiceConfig") { [weak self] params in
            guard let self else { throw JsonRpcDispatchError.internalError("Helper is shutting down") }
            let object = (params as? [String: Any]) ?? [:]
        ...
        router.register("cuelive.speak") { [weak self] params in
            guard let self else { throw JsonRpcDispatchError.internalError("Helper is shutting down") }
            let object = (params as? [String: Any]) ?? [:]
            return self.ensureCueLive().speak(params: object)
        }
        router.register("cuelive.performAction") { [weak self] params in
            guard let self else { throw JsonRpcDispatchError.internalError("Helper is shutting down") }
            guard let object = params as? [String: Any] else {
                throw JsonRpcDispatchError.invalidParams("cuelive.performAction requires params")
            }
            return self.ensureCueLive().performAction(params: object)
        }
