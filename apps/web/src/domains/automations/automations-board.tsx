 ...
 * #F4F3EF). Two columns: Watchers (source · interval · a health chip that
 * carries a glyph as well as a colour · what it watches · hits vs. checks) and
 * Playbooks (trigger→action · autonomy chip · priority · last-fired). A
 * global-trust banner makes the autonomy-vs-dial relationship explicit; the
 * autonomy the server capped is shown with a 🔒. Create is inline. All data +
 * the cap come from the daemon (no fake state) — including the hit counts,
 * which the card must never substitute the poll clock for.
 */
...
function WatcherCard({ watcher }: { watcher: Watcher }) {
  ...
  return (
    ...
    >
      ...
      {/* What it is pointed at. A watcher whose card never said this read as
          "on, but pointing nowhere" — which is the same sentence whether it is
          watching an entire account or nothing at all. */}
      {watcher.scope ? (
        <div
          style={{
            fontFamily: sans,
            fontSize: 12.5,
            color: watcher.scope.watching ? MUTED : RED,
            marginTop: 9,
            lineHeight: 1.5,
          }}
        >
          {watcher.scope.watching ? "Watching: " : ""}
          {watcher.scope.summary}
        </div>
      ) : null}
