...
export function resolveAnswers(answered: AnsweredQuestion): ResolvedAnswer[] {
  ...
  return answered.questions.map((question) => {
    ...
    // answer, so the only way one reaches here is the legacy single-question
    // wire shape, which has no `skip` kind and coerces a skip to `free_text`
    // with empty text. Reading it as free text would render a blank answer row.
