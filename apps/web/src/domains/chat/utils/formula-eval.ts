 ...
 * produces.
 *
 * This is the browser port of the daemon's `spreadsheet-studio/tools/formula-eval.ts`.
 * It is pure TypeScript (no node/ExcelJS deps) and deliberately kept identical in
 * vocabulary to the daemon engine — the same functions and operators the
 * generator emits — so the in-Cue spreadsheet viewer can recalculate edited
 * cells live, entirely client-side, without shipping a heavyweight formula
 * engine.
 *
 * Why the vocabulary matches the generator: `spreadsheet_create` writes formula
 * cells as ExcelJS `{ formula }`; the daemon caches results with this same
 * engine so a delivered file carries real numbers. When the user edits a cell
 * in the viewer, we re-run `computeResults` over the whole workbook here to
 * refresh every dependent formula.
 *
