 ...
 * Ported from upstream vellum-assistant (48a63d28d7).
 *
 * ## Why this is not the system camera
 *
 * The obvious move (`<input type="file" capture="environment">`, which raises
 * the OS camera) is wrong for this feature in three ways: it is modal and
 * one-shot, so every photo costs a full open/aim/expose cycle; it covers the
 * room, so the call it belongs to disappears while you use it; and on a phone
 * it puts the OS in charge of an audio session a call is currently holding. A
 * viewfinder the room owns is a `<video>` element, which does none of that and
 * stays open across as many photos as the user wants to take.
 *
 ...
 * with `echoCancellation` attached to it — the thing the whole echo-safe
 * playback program depends on. This module therefore requests `{ video: … }`
 * and NOTHING else. Asking for `{ audio: true, video: true }` would hand back
 * a second, differently-configured audio track and tear down the capture
 * graph the call is running on: a call that goes deaf, or the return of the
 * echo bug the AEC routing fixed.
 *
 * ## Permissions
 *
 * `openCamera()` calls `getUserMedia` directly, so the tap that opens the
 * camera is the thing that raises the OS permission alert — nothing
 * dismissible sits between the two, which is why the room offers a plain
 * camera button rather than an explanatory sheet.
 */
...
/**
 * JPEG quality for a captured frame. A viewfinder frame is only as big as the
 * negotiated video track, so this lands well under the attachment pipeline's
 * auto-resize threshold and `prepareImageAttachmentForUpload` usually passes
 * it through untouched. Quality is chosen for a model reading the image, not
 * for a photo library.
 */
const CAPTURE_JPEG_QUALITY = 0.85;

/**
 * Whether a viewfinder can run in this environment. Pure feature detection —
 * the desktop app's Electron shell grants video capture to the trusted
 * renderer origin (see apps/macos/src/main/permissions.ts), so there is no
 ...
 * collapse (and the end of the call) turn the camera hardware off without
 ...
 * could not read plain state like `open` or `facing` during render without
 * being flagged.
 */
  ...
  const acquire = useCallback(
    async (nextFacing: VoiceCameraFacing): Promise<VoiceCameraError | null> => {
      ...
      try {
        ...
        // renegotiate the microphone the call is already streaming from and
        // break the echo-cancellation the call depends on.
        //
        ...
        // OverconstrainedError; the `ideal` resolution asks for a frame a
        // model can read text from, and degrades silently on lesser cameras.
