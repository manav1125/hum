 ...
 * call can see. Ported from upstream vellum-assistant (48a63d28d7).
 *
 * Deliberately made of parts that already exist. A captured frame is a `File`,
 * which is exactly what a typed message's attachment is, so it goes through
 * the same two steps the composer's paperclip uses,
 * {@link prepareImageAttachmentForUpload} then {@link uploadChatAttachment},
 * with the same HEIF/size handling and the same transcript rendering. Only
 * the last step is voice-specific: the returned id is handed to the
 * live-voice session ({@link attachLiveVoiceImage}), which persists it into
 * the conversation as its own user message and runs no turn for it.
 *
 ...
 * plain: it is read at a glance, over a live call, by someone holding a
 * device up at something.
 */
const CAMERA_ERROR_COPY: Record<string, string> = {
  "permission-denied": "Camera access is off for Cue.",
...
export interface VoiceRoomCamera {
  ...
  /** Open the viewfinder. Call directly from a tap (OS permission alert). */
...
export function useVoiceRoomCamera(
  ...
  // Release every preview on unmount. The room unmounts on collapse and on
  // the end of the call, so this is the common path, not the edge case.
  useEffect(
    () => () => {
      setPhotos((current) => {
  ...
  const shutter = useCallback(async () => {
    ...
    try {
      ...
      // high-resolution track on a recent device) behaves like every other
