 ...
 * renders without a History section. That lets same-source self-hosted setups
 * (which report the last released version while running unreleased code) get
 * the section without a debug override.
 *
 ...
 * treat them the same: the detail page shows the History section only when
 * there are revisions to show, which both cases fail for the same reason from
 * a reader's point of view.
 *
 * Ported from upstream 4fe79f4a13 + 7eb20048ac.
 */
 ...
 *
 * Other HTTP failures throw a status-carrying {@link ApiError}: the
 * `throwOnError: false` read bypasses the client's error interceptor, so the
 * status has to be attached here for callers to distinguish 4xx from 5xx. A
 * network error (no response at all) rethrows raw so it retries as a
 * transient failure.
 */
export async function fetchSkillHistory(
  ...
  if (!response || !response.ok) {
    ...
    if (response) {
      throw new ApiError(
        response.status,
        extractErrorMessage(error, response, "Failed to fetch skill history."),
      );
    }
...
/**
 * Whether the skill detail page should offer its History section.
 *
 * A section is a promise that there is something behind it, so an empty
 * result hides it entirely rather than opening onto an empty state. A FAILED
 ...
 */
export function shouldShowHistorySection(state: {
  isLoading: boolean;
  isError: boolean;
