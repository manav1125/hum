...
// PLACEHOLDER DESIGN (approved): every literal hex below is upstream's value,
// carried over verbatim as placeholder colors for the ported concept graph.
// SWAP POINT — when Cue's own graph palette is decided, update
// `NODE_KIND_COLORS`, `CLUSTER_PALETTE`, and `EDGE_LEARNED_COLOR` here (this
// file is the only place literal graph hexes live; the canvas renderer can't
// read CSS `var()`, which is why they're literals at all).
