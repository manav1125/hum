...
export function ConceptGraphView({
  ...
  if (query.isLoading) {
    ...
    // Upstream renders a guided upgrade prompt here (enable Memory / migrate
    // to v3). This fork's graph is served off the live memory v2 substrate,
    // so `unsupported` only means Memory itself is switched off — and the
    // guided upgrade skill doesn't exist here, so no dead buttons: plain copy.
    body = (
      <CenteredMessage
        title="Memory is turned off"
        detail="Turn Memory back on in Settings and your assistant's concept map will grow here."
      />
    );
  } else if (!ready) {
    body = (
