...
# Cue: sub-path mount (see next.config.ts). basePath is compiled into the
# build, so the ARG must reach `pnpm build`.
ARG OPENMAIC_BASE_PATH
ENV OPENMAIC_BASE_PATH=$OPENMAIC_BASE_PATH
# Cue: cap the V8 heap below the builder VM's RAM so the Next/Turbopack
...
# Cue: writable data dir for classroom/job persistence and usage records —
# the app runs as nextjs and /app is root-owned, so without this every
# write logs EACCES and async classroom jobs can't persist.
