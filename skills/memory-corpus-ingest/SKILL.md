...
After ingest, the enqueued reindex jobs pick up the new pages and embed them; the map becomes retrievable shortly after, with each page's declared `origin_date` recorded so recency-aware ranking treats the content as dating from when it happened rather than the import day.
...
If a query routes to nothing, the relevant lead is not doing its card job; rewrite it and re-ingest that page with `--overwrite`. The query checks above are the verification.
...
## Foreign-assistant memory parsers

When the corpus is another assistant's memory dump rather than a document archive, two deterministic parsers under `scripts/` turn the source material into `MemoryImportItem[]` JSON review candidates (never final memories; the creator reviews before anything is staged):

```bash
# ChatGPT account export ZIP: saved memories + custom instructions only
# (conversation history belongs to the chatgpt-import skill).
bun run {baseDir}/scripts/parse-chatgpt-memory.ts --file /path/to/chatgpt-export.zip

# Hermes / OpenClaw memory.db SQLite snapshot (point at a .backup copy,
# never a live DB). Credential-named columns are excluded and well-known
# secret shapes are redacted in place before anything reaches stdout.
bun run {baseDir}/scripts/parse-agent-memory-db.ts --file /path/to/snapshot.db --source hermes
```

Both print a census to stderr and stream items JSON to stdout. After creator review, distill the accepted items into staged concept pages (carrying `source:` and `origin_date:` frontmatter from each item's provenance) and land them with `assistant memory ingest` exactly like map pages.
