        ...
        fetchServerProviders: async () => {
          try {
            ...
            set((state) => {
              ...
              // Cue Learn: browser-native speech synthesis is effectively
              // silent in the embedded classroom (it reports speaking without
              // audible output in several webview/iframe contexts), so when
              // the deployment configures a real TTS provider server-side,
              // promote it over a still-on-default selection. ttsFallback
              // already lists server-configured providers first. A user can
              // still pick any configured provider in Settings; this only
              // moves the selection off browser-native.
              if (validTTSProvider === ('browser-native-tts' as TTSProviderId)) {
                const serverManaged = ttsFallback.find(
                  (pid) =>
                    newTTSConfig[pid]?.isServerConfigured &&
                    !newTTSConfig[pid]?.serverDisabled,
                );
