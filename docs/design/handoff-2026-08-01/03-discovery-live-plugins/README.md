...
- **L2 — Web session live:** SAME route becomes the remote viewer — green "Live on <device>", mirrored screen with Cue's cursor + move-label, rail with intent/pause/take-over verbs matching the Mac overlay. Web steers, never holds grants. (NEEDS BACKEND: screen stream + input relay.)
...
Web never gates Cue Live · discovery names the cost before you tap · plugins are opt-in with disable/remove always present · NEEDS BACKEND: app deep-link/handoff, web live-stream + input relay, Cue-curated registry.
