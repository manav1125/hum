 ...
 *
 * There is **no id-prefix fallback**. The documented prefix for an MPIM is
 * `G`, but `G` is shared with private channels, and modern workspaces mint
 * MPIMs with a plain `C` prefix (`is_mpim: true, is_channel: true`). An id
 ...
 * own: it composes it with the observed-kind cache above.
 */
...
/** Exported for testing — clears the channel info and kind caches. */
 ...
 * etiquette for it. Reporting `im` for a multi-party room would suppress
 * that etiquette.
 *
 * Unlike `normalizeSlackDirectMessage` there is no unconditional
 * default-assistant fallback: a 1:1 IM is by construction addressed to the
 * bot, but an MPIM is multi-party and can include participants with no
 * routing relationship, so routing follows the standard chain
 * (conversation_id / actor_id / unmappedPolicy).
 *
 * Threading matches DMs, not channels: an MPIM is one continuous
 * conversation, so a top-level message must not fall back to its own ts as a
 * thread id, or every message would resolve a separate daemon conversation.
 *
 * Bot's own messages are dropped by `processEventPayload` before
 * normalization.
 */
