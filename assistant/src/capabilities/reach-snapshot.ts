  ...
  if (snapshot.userBrowser.reachable) {
    lines.push(
      snapshot.userBrowser.route === "extension"
        ? "- The user's own Chrome: REACHABLE via the connected Chrome extension. The `browser_*` tools (browser_navigate, browser_click, browser_fill, browser_extract, …) drive their real browser, with their real logged-in sessions. For ANY web task — opening a site, filling a web form, reading a page, \"do X on this website\" — use the `browser_*` tools. Do NOT use `computer_use_*` screen-control to click around a browser: that is the wrong tool for the web and hijacks their screen. `computer_use_*` is for native desktop apps only."
        : "- The user's own Chrome: REACHABLE via the desktop bridge. The `browser_*` tools (browser_navigate, browser_click, browser_fill, browser_extract, …) drive their real browser, with their real logged-in sessions. For ANY web task — opening a site, filling a web form, reading a page, \"do X on this website\" — use the `browser_*` tools. Do NOT use `computer_use_*` screen-control to click around a browser: that is the wrong tool for the web and hijacks their screen. `computer_use_*` is for native desktop apps only.",
    );
  } else {
    lines.push(
