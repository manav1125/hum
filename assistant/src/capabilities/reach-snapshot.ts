  ...
  if (snapshot.userBrowser.reachable) {
    lines.push(
      snapshot.userBrowser.route === "extension"
        ? "- The user's own Chrome: REACHABLE via the connected Chrome extension. Browser tools drive their real browser, with their real logged-in sessions."
        : "- The user's own Chrome: REACHABLE via the desktop bridge. Browser tools drive their real browser, with their real logged-in sessions.",
    );
  } else {
    lines.push(
