 ...
 * resolve against. The workspace-level shim covers only the plugin-api itself
 * (`@vellumai/plugin-api`, see `../../plugins/ensure-plugin-api-shim.ts`);
 * anything a plugin brings itself was unresolvable.
 *
 ...
 *   no lockfile is written, so the only artifact is `node_modules/`, which
 *   every fingerprint walk already excludes (see
 *   `EXCLUDED_DIRS_ANYWHERE` in {@link ./plugin-fingerprint}).
 *
 ...
 * plugins that import only the plugin-api.
 */
