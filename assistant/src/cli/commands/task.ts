...
Examples:
  ...
  $ assistant task delete task_abc123 --json`,
      ...
      task
        .command("dedupe")
        .description(
          "Collapse existing exact-duplicate task templates and work items",
        )
        .option("--json", "Output result as machine-readable JSON.")
        .addHelpText(
          "after",
          `
Collapses existing exact-duplicate task templates and active work items into
one canonical row per group, keeping the OLDEST of each group. Non-duplicate
data is never touched, and the command is idempotent (a second run is a no-op).

Examples:
  $ assistant task dedupe
  $ assistant task dedupe --json`,
        )
        .action(async (opts: { json?: boolean }) => {
          const result = await cliIpcCall("task_dedupe", { body: {} });
        ...
        .action(
          async (opts: {
            ...
            if (opts.priority !== undefined)
              params.priority_tier = opts.priority;
            if (opts.sortIndex !== undefined)
              params.sort_index = opts.sortIndex;
