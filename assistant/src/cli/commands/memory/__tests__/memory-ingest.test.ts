...
// The CLI lazily imports the batch cap from the daemon route module. The
// real module is spread (per the mock.module rule) and only the ingest
// service is stubbed out of its dependency chain above via cli-client; the
// cap itself is re-exported unchanged so the chunking tests track the real
// value instead of a hardcoded copy.
const actualIngestRoutes =
  await import("../../../../runtime/routes/memory-ingest-routes.js");
mock.module("../../../../runtime/routes/memory-ingest-routes.js", () => ({
  ...actualIngestRoutes,
}));
...
function buildProgram(): Command {
  ...
  registerMemoryIngestCommand(program);
  return program;
}
