 ...
 *
 * The pending counterpart (`question_request` / the pending-interaction
 * registry entry) is live state: it exists only while the prompt is
 * outstanding and vanishes the moment the interaction resolves. This record
 * is the opposite. It is produced by the `ask_question` executor once the
 * prompt settles, rides the `tool_result` event, and is persisted on the
 * tool_use block, so the answered question and the user's choice stay
 * readable in the transcript across conversation switches, reloads, and
 * history reopens.
 *
