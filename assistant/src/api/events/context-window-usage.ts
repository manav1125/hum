 ...
 * measured with the provider's own tokenizer (falling back to the local
 * estimate when the provider has no count endpoint). Emitted by
 * user-initiated compaction (`/compact`, "summarize up to here"), which
 * recomputes the count outside any turn and so has no `usage_update` to
 * carry it. Clients apply it to the context-window indicator, which
 * otherwise only refreshes on the next LLM call.
 *
