...
// Ported from upstream 4fe79f4a13 ("recent revisions on the skill detail
// page"), adapted to this tree's WorkspaceGitService.
// ---------------------------------------------------------------------------
//
...
//
// Every git call goes through `WorkspaceGitService.runReadOnlyGit`, whose
// underlying `execGit` centrally splices `--no-textconv --no-ext-diff` into
// diff/show/log invocations (the S4 guard): workspace files and git metadata
// are model-writable, so without those flags merely viewing a diff could run a
// repository-controlled textconv/ext-diff helper inside the daemon.
//
// This module is reached from an HTTP GET, which `src/runtime/AGENTS.md`
// requires to be side-effect-free, and `runReadOnlyGit` awaits
// `ensureInitialized()`, which would CREATE the repository (git init,
// .gitignore, hooks, an initial commit) on a workspace that is not yet a repo.
// Observing history must never be the thing that brings a repository into
// existence, so this module checks for `.git` itself and degrades to empty
// history when the workspace is not a repository. On an existing repo,
// `ensureInitialized` only performs the idempotent normalization every other
// daemon write already triggers once per process.
...
/**
 * Per-revision diff cap, in characters. A pathological revision (a generated
 * asset checked into a skill, say) should not make one history entry dwarf the
 * whole response; the entry survives with its file list and a truncation
 * marker, and the full diff remains reachable through the assistant itself.
 */
export const MAX_REVISION_DIFF_CHARS = 120_000;

/** Marker appended when a revision's diff was cut at the cap. */
...
/** Cut an oversized diff at the cap, on a line boundary, with a marker. */
function capDiff(diff: string): string {
  if (diff.length <= MAX_REVISION_DIFF_CHARS) {
    return diff;
  }
  const cut = diff.lastIndexOf("\n", MAX_REVISION_DIFF_CHARS);
  return (
    diff.slice(0, cut > 0 ? cut : MAX_REVISION_DIFF_CHARS) +
...
export async function getSkillHistory(
  ...
  // Reached from a GET: never let observing history create the repository.
  // `runReadOnlyGit` initializes an absent repo, so the existence check has
  // to happen here, before the service is touched.
  ...
  try {
    git = getWorkspaceGitService(workspaceDir);
  } catch (err) {
  ...
  for (const { sha, changedAt } of shas) {
    ...
    revisions.push({ id: sha, changedAt, files, diff: capDiff(diff) });
  }
