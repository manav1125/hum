 ...
 * them straight into `memory/concepts/` via the v2 substrate's `ingestPages`,
...
export async function handleMemoryIngest(
  ...
  if (!config.memory.v2.enabled) {
    throw new BadRequestError(
      "Concept-page memory is not active - enable memory.v2.enabled to ingest pages.",
    );
  }
  ...
  if (!parsed.success) {
    ...
    throw new BadRequestError(
      `Invalid request body${path ? ` at ${path}` : ""}: ${issue?.message ?? "validation failed"}`,
    );
  }
  ...
  try {
    ...
    // batches before `ingestPages` runs; anything else that escapes
    // `ingestPages` is an internal error for the adapter to surface.
    throw err;
  }
}
