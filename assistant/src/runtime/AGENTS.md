...
- **Resolution**: Clients respond via standalone endpoints keyed by `requestId`:
  - `POST /v1/confirm` — `{ requestId, decision, selectedPattern?, selectedScope? }`. Valid decisions: `"allow"`, `"allow_10m"`, `"allow_conversation"`, `"deny"`. The temporary-grant verbs (`allow_10m`, `allow_conversation`) approve the current action and install an in-memory per-conversation override (`runtime/conversation-approval-overrides.ts`) that auto-approves subsequent eligible prompts — never checkpoint-forced (send/money/delete-class), `requireFreshApproval`, or voice-session prompts, and expiry always returns to ask. The response echoes `approvalOverride: { kind, conversationId, expiresAt }` for grant decisions so clients can render a countdown. Persistent rules are created via `POST /v1/trust-rules`, not via confirm decisions.
  - `GET /v1/approval-override?conversationId=…` — live temporary-override status (`{ active, kind?, expiresAt?, remainingMs? }`); `POST /v1/approval-override/clear` — `{ conversationId }`, revokes the override (idempotent).
