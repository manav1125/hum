...
export const skillLoadTool = {
  ...
  description:
    'Load full instructions for a skill. Works for both bundled skills (listed in the catalog) and custom workspace skills. Bundled/first-party skills (like `app-builder`) are already installed — loading only activates their instructions for the current conversation, since skills unload between turns; treat "load" as activation, not installation, and don\'t tell users a bundled skill needs installing. Loading can still fail (e.g. a feature-gated skill returns "currently unavailable", or the load errors) — if it does, relay that specific error rather than claiming the skill is not installed or offering to install it. For app, website, dashboard, game, calculator, tracker, visualization, slide deck / presentation / pitch deck, or interactive tool requests, load `app-builder` with `skill: "app-builder"` — for decks, build the slides as an app and export a shareable file with `deck_export_pdf` when the user wants to send or present it. For writing or drafting ANY document or multi-paragraph written piece — a letter (cover/business/formal/reference/application), memo, proposal, report, brief, essay, article, blog post, plan, statement, contract draft, or similar — load `document-editor` with `skill: "document-editor"` and create it with `document_create` (which opens the rich-text editor panel); do NOT reply with the document inline in chat or write it to a scratch/temp file; when the user wants the document as a shareable/printable FILE, export it with `document_export_pdf`, and for a standalone PDF (invoice, one-pager, flyer) use `pdf_create`. For producing a COMPLETE video — an explainer, promo, ad, trailer, montage, slideshow, social reel/short, or any multi-scene and/or narrated video — load `video-studio` with `skill: "video-studio"`, which runs the full pipeline (generate scenes/voiceover/music with `replicate_run`, then assemble with `video_compose`); for a single raw generated clip with no script/editing, use `replicate_run` directly instead. For generating or editing an image, load `image-studio` (`media_generate_image`); on self-hosted deployments where the managed proxy is unavailable, `replicate_run` (via the `replicate` skill) is the working direct image path, and `media_generate_image` automatically falls back to it when a Replicate token is configured. If an image/media error states a Replicate token IS configured, that is a fresh live result — NEVER assert from remembered earlier failures that Replicate is broken without retesting it in the current turn. For adding, checking, running, or completing items on the user\'s to-do/task list ("add to my tasks", "what\'s on my plate", "run that task", "mark it done"), load `tasks` with `skill: "tasks"` and use its `task_list_add`/`task_list_show`/`task_list_update`/`task_queue_run` tools — NEVER manage the task queue via bash/CLI commands. For a spreadsheet, Excel file, financial model, budget, forecast, P&L, cap table, or any .xlsx deliverable, load `spreadsheet-studio` with `skill: "spreadsheet-studio"` and build it with `spreadsheet_create` using LIVE formulas (never precomputed values); NEVER hand-build the xlsx via bash/host_bash (python openpyxl/pandas/zipfile, node, apt, npm) or by writing raw XML — that produces a download-only file that will not open in the native viewer and floods the user with per-command approval prompts; and do NOT paste CSV or giant markdown tables in chat instead.',
