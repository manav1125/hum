...
export const uiShowTool = {
  ...
  description:
    ...
    '- card: { title, subtitle?, body, metadata?: [{ label, value }], template?, templateData? }. Templates: "weather_forecast" (native weather widget), "task_progress" (live step tracker - update via ui_update on data.templateData; shape: { title, status: "in_progress"|"completed"|"failed", steps: [{ label, status: "pending"|"in_progress"|"completed"|"failed", detail? }] }), "skill_recommendations" and "channel_showcase" (below)\n' +
    '- card template "skill_recommendations": templateData: { intro?, skills: [{ id, title, iconKey?, description, prompt, capabilities?: [string], requirements?: [{ label, status: "ready"|"connect", hint? }] }] }. When the user asks what you can do / how you can help, answer with THIS card (3-5 skills, strongest-for-this-user first), not a prose list. Compose rows yourself from the bundled skill catalog and what you know about the user: `id` is the skill id, `title` a human name, `description` one concrete line of what it does for THEM, `prompt` a ready-to-send Try-me message written in the user\'s voice (e.g. inbox-management -> "Declutter my inbox", start-the-day -> "Give me my morning briefing", github -> "Review my open pull requests", research -> "Research a topic for me"). `capabilities` are 2-4 short "things we can do" bullets; `requirements` lists what the skill needs, status "ready" when already satisfied (e.g. connector linked) or "connect" when setup is needed. iconKey one of: "inbox" | "sun" | "github" | "search" | "calendar" | "mail" | "globe" | "file-text" | "message-square" | "sparkles" (defaults to "sparkles"). Keep body a one-line prose fallback. Non-blocking: never attach actions or await on this card.\n' +
    '- card template "channel_showcase": templateData: { intro?, channels: [{ id, status: "live"|"available", label? }] }. When the user asks HOW they can reach you / talk to you / what channels exist, answer with THIS card, not a prose list. Build rows from your "# Your Channels" section: every connected channel gets status "live"; the other channels you support (slack, telegram, whatsapp, phone, sms, email, a2a) get status "available". `id` must be one of those channel ids; the client supplies icons, labels, setup buttons, and the Desktop/Web/iOS/CLI interface rows — do not add interface rows yourself. Keep body a one-line prose fallback. Non-blocking: never attach actions or await on this card.\n' +
