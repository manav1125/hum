 ...
 * escaped, and the wrapper parses back to a well-formed envelope. The
 * tool's own voice — backend disclosure, login guidance — stays outside
 * the fence.
 */
...
// ---------------------------------------------------------------------------
// Module mocks (must be declared before dynamic import). Spread the real
// module and override only the seam under test — an exhaustive factory
// would silently delete every other export for every later file in the
// run (see assistant/CLAUDE.md).
// ---------------------------------------------------------------------------

const actualFactory = await import("../cdp-client/factory.js");
...
// The real browser manager is used (mocking it globally would leak into
// every other test file in the run); the fake client's backend-kind memo
// and snapshot maps are per-conversation, so nothing needs seeding.
const {
...
const ENVELOPE_RE = /<external_content[^\n]*>\n[\s\S]*?\n<\/external_content>/g;
 ...
 * challenge separately from its own scaffolding lines; snapshot and
 * extract, which append backend-disclosure lines after the fence).
 */
function extractEnvelopes(content: string): string[] {
  return content.match(ENVELOPE_RE) ?? [];
}
 ...
 * closed the fence early). Tool-voice lines outside the fence (backend
 * disclosure) are allowed.
 */
function expectSingleEnvelope(content: string) {
  expect(content.match(/<external_content/g)?.length).toBe(1);
  const blocks = extractEnvelopes(content);
  expect(blocks).toHaveLength(1);
  const envelope = parseExternalContentEnvelope(blocks[0]!);
...
// Container-browser login guidance is the tool's own voice and must stay
// outside the fence (see `loginGuidanceLines` in browser-execution.ts).
const LOGIN_GUIDANCE_SNIPPET = "This login wall is in a browser";
...
describe("browser_snapshot fences page-derived content", () => {
  ...
  test("keeps the backend disclosure outside the fence", async () => {
    currentPage = makeFakePage();

    const result = await executeBrowserSnapshot(
      {},
      makeContext("snap-disclosure"),
    );

    const closeIndex = result.content.lastIndexOf("</external_content>");
    const disclosureIndex = result.content.indexOf("Browser used:");
    expect(closeIndex).toBeGreaterThan(-1);
    expect(disclosureIndex).toBeGreaterThan(closeIndex);
  });
...
describe("browser_navigate fences page-derived content", () => {
  ...
  test("fences the auth challenge while leaving login guidance outside", async () => {
    currentPage = makeFakePage({
      title: "Sign in",
      authChallenge: {
        type: "login",
    ...
    // The tool's login guidance must stay in its own voice, after the
    // fenced challenge.
    expect(result.content).toContain(LOGIN_GUIDANCE_SNIPPET);
    expect(result.content.indexOf(LOGIN_GUIDANCE_SNIPPET)).toBeGreaterThan(
      result.content.indexOf("</external_content>"),
    );
  });
