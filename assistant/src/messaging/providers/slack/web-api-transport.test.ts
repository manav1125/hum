...
describe("SlackApiError", () => {
  test("derives status from category when not explicit", () => {
    ...
    expect(new SlackApiError("account_inactive").status).toBe(401);
    expect(new SlackApiError("org_login_required").status).toBe(401);
...
describe("rawSlackRequest", () => {
  ...
  test("retries body-level 'ratelimited' spelling, then succeeds", async () => {
    let calls = 0;
    globalThis.fetch = mock(async () => {
      calls++;
      if (calls === 1) {
        return jsonResponse({ ok: false, error: "ratelimited" });
      }
      return jsonResponse({ ok: true });
    }) as unknown as typeof fetch;
