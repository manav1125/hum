/**
 * Tests for the VoiceFrontDecider presence-layer service (WS-E).
 *
 ...
 * abort) resolves to `null` within the configured budget, so the caller's
 * static fallback (or silence) always covers it. Endpointing tests live with
 * the unified front door (live-voice-vad / voice-triage-escalate), which
 * replaced the decideEndpoint LLM call.
 */
...
describe("createVoiceFrontDecider — generateProgressText", () => {
      ...
      completedOps: [
        {
          ...
          // appended closing delimiter would complete it, and a model
  ...
  test("missing/foreign tool block → null", async () => {
    const decider = createVoiceFrontDecider({
      ...
      getProvider: async () =>
        stubProvider(async () =>
          toolResponse({ update: "On it." }, "other_tool"),
        ),
    });
    expect(await decider.generateProgressText(progressInput)).toBeNull();
  });
