/**
 * Session-start gating on the live-voice credential preflight (WS-E).
 *
 * The preflight resolver is injected as a stub — the resolver's own readiness
 * logic is covered in live-voice-credential-preflight.test.ts. These tests pin
 * the wiring: a not-ready verdict rejects the session at the start frame with a
 * `credentials_unavailable` error frame carrying the preflight's user message,
 * before any transcriber is resolved, and leaves the session manager free for a
 * retry. No DB is touched — the rejection happens before conversation setup.
 */
...
// The "ready"/"null" paths proceed past the preflight into conversation
// setup, so the conversations table must exist. Initialize the per-process
// test DB up front (mirrors live-voice-thread.test.ts) so this file passes in
// CI's per-file isolation, not only in a shared-process whole-dir run.
initializeDb();
...
describe("live-voice session credential preflight gating", () => {
  ...
  test("an explicit null preflight resolver skips the gate entirely", async () => {
    const { context, frames } = createContext();
    const session = new LiveVoiceSession(context, {
      resolveTranscriber: mock(async () => new MockStreamingTranscriber()),
      // `null` force-disables the preflight even when config would enable it.
