...
describe("LiveVoiceSession archive and metrics events", () => {
  test("archives user and assistant audio and emits completion and session metrics", async () => {
    ...
    expect(completedMetrics).toMatchObject({
      ...
      turnId: "live-turn-1",
      sttMs: 10,
      // The assistant-dispatch mark sits between final transcript and first
      // delta on the stepped test clock, so the transcript-anchored figure
      // spans two ticks while the dispatch-anchored one spans one.
      llmFirstDeltaMs: 20,
