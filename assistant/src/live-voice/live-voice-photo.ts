 ...
 *
 * Ported from upstream vellum-assistant (48a63d28d7, 639f7bc1cb) — the FINAL
 * persist-immediately design, not the abandoned ride-the-next-turn draft.
 *
 ...
 * image.
 *
 * The alternatives were both worse and both were tried upstream. Attaching the
 * photo to the *next* spoken turn strands it whenever the user speaks first:
 * the question "what's this?" dispatches without an image and the photo waits
 * for a sentence that has already been said. Dispatching a turn *for* the
 * photo races the sentence the user is in the middle of saying, so the
 * assistant answers the picture and the words separately.
 *
 ...
 * under overflow.
 *
 * (Vision routing needs no work here: our vision tier reroutes image-bearing
 * history to the vision model automatically once the row carries an image.)
 */
 ...
 * Mirrors the HTTP send path's own attachment resolution, so a photo taken
 ...
 * do: they are the existing precedent for "persist a user message without the
 * agent loop", and the lock is what keeps this write from interleaving with a
 * turn's own persist.
 *
 * Never throws. A photo that cannot be stored must not take the call down with
 * it, and the caller reports the failure to the user instead (a non-fatal
 * `error` frame with `frameType: "attach_image"`).
 */
export async function persistLiveVoicePhoto(
  ...
  try {
    ...
    try {
      ...
      recordPersistedSeq(conversationId, getCurrentSeq());
      ...
      // turn ends.
      void conversation.drainQueue("loop_complete");
    }
  } catch (err) {
    log.warn(
