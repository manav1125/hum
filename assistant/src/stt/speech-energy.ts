 ...
 *
 * `referenceDownsampleFactor` (defaults to `downsampleFactor`) lets a
 * reference recorded at a different sample rate be compared against the
 * input: pick factors so both block-average to the same effective rate
 * (e.g. 16 kHz input at 8 vs. a 24 kHz reference at 12 → 2 kHz each).
 */
export function pcm16MaxNormalizedCorrelation(
  input: Buffer,
  reference: Buffer,
  downsampleFactor = 8,
  referenceDownsampleFactor = downsampleFactor,
): number {
  if (
    !Number.isInteger(downsampleFactor) ||
    downsampleFactor <= 0 ||
    !Number.isInteger(referenceDownsampleFactor) ||
    referenceDownsampleFactor <= 0
  ) {
    return 0;
  }

  let inputSamples = blockAveragePcm16(input, downsampleFactor);
  const referenceSamples = blockAveragePcm16(
    reference,
