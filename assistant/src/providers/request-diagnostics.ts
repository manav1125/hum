...
const log = getLogger("request-diagnostics");
...
/**
 * Ceiling on the error-body excerpt included in the failure log line — the
 * full body stays available on the diagnostics record itself.
 */
const LOGGED_ERROR_BODY_CHARS = 2_048;
...
/** True when the current async context already carries a diagnostics scope. */
export function hasActiveDiagnosticsScope(): boolean {
  return recorderStorage.getStore() !== undefined;
}
...
/**
 * Run one provider send inside a diagnostics scope and, when it fails, log
 * everything observed on the wire (redacted URL, model, connection, HTTP
 * status, upstream error body head) before rethrowing.
 *
 * This is the fork's minimal surfacing of the diagnostics record: the
 * provider-dispatch wrappers (`CallSiteRoutingProvider`,
 * `CallSiteConfiguredProvider`) wrap their sends with it so every failed
 * request leaves actionable evidence in the daemon log. When an outer caller
 * already established a scope (e.g. a route collecting evidence for its own
 * response), this delegates untouched so the outer scope keeps observing —
 * nesting would shadow the outer recorder and empty its evidence.
 */
export async function withProviderRequestDiagnosticsLogging<T>(
  fn: () => Promise<T>,
): Promise<T> {
  if (hasActiveDiagnosticsScope()) {
    return fn();
  }
  const attempt = await runWithProviderRequestDiagnostics(fn);
  if (attempt.ok) {
    return attempt.value;
  }
  const { upstream_error_body, ...rest } = attempt.diagnostics;
  log.warn(
    {
      ...rest,
      ...(upstream_error_body !== undefined
        ? {
            upstream_error_body_head: upstream_error_body.slice(
              0,
              LOGGED_ERROR_BODY_CHARS,
            ),
          }
        : {}),
      error:
    ...
    "provider request failed — outbound request diagnostics",
  );
  throw attempt.error;
}
