 ...
 * plus `"multi"` itself); when the web settings surface grows a language
 * catalog (apps/web), it should mirror this list with a parity test pinning
 * the two together. Extending it requires verifying nova-3 monolingual
 * support in Deepgram's docs first. Regional variants (e.g. "en-US") stay
 * out: `services.stt.language` accepts any non-empty string, so they remain
 * expressible as custom values.
 */
