...
describe("withProviderRequestDiagnosticsLogging", () => {
  test("rethrows the original failure after logging", async () => {
    // GIVEN a send that fails after reaching the upstream
    const boom = new Error("provider exploded");

    // WHEN it runs under the logging wrapper with no outer scope
    const attempt = withProviderRequestDiagnosticsLogging(async () => {
      await fetch(`${origin}/verbose-404`);
      throw boom;
    });

    // THEN the caller still observes the original error
    await expect(attempt).rejects.toBe(boom);
  });

  test("reuses an outer diagnostics scope instead of shadowing it", async () => {
    // GIVEN an outer scope collecting evidence for its own response
    const outer = await runWithProviderRequestDiagnostics(async () => {
      // WHEN a wrapped send runs inside it
      await withProviderRequestDiagnosticsLogging(async () => {
        recordProviderRequestDiagnostics({ model_id: "gpt-5.6-sol" });
        await fetch(`${origin}/verbose-404`);
      });
      return "done";
    });

    // THEN the outer scope keeps observing everything the send recorded
    expect(outer.ok).toBe(true);
    expect(outer.diagnostics.model_id).toBe("gpt-5.6-sol");
    expect(outer.diagnostics.http_status).toBe(404);
    expect(outer.diagnostics.upstream_error_body_state).toBe("captured");
  });
});
