...
export async function startVoiceTurn(
  ...
  // carries its routing verdict before it carries any speech, so hub
  // subscribers (web chat, passive devices) read it through the gate and see
  // only the text the caller heard. Every other leg — including the
  // escalated continuation that produces the real reply, and every leg on
  // the flag-off path, which never sets `routingLeg` — keeps broadcasting
  // its deltas untouched, by object identity.
   ...
   * leg's control-plane text back AT THE BOUNDARY rather than emitting it
   * and repairing the transcript afterwards. Text released by the gate
   * travels as an ordinary delta on the leg's own reserved row, so a client
   * rendering the stream lands on the same text the teardown hygiene pass
   * persists.
   */
  const broadcastLegEvent = (msg: ServerMessage): void => {
      ...
      await conversation.runAgentLoop(persistedContent, messageId, {
        onEvent: (msg: ServerMessage) => {
          ...
          if (frontDoorStreamGate !== null && msg.type === "message_complete") {
            ...
            // and an answer leg's held "[…"-tail is real text — release
            // either ahead of the completion frame. A cancelled leg never
            // hands off, and correspondingly never flushes.
