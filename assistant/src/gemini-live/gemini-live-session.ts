...
function buildSystemInstruction(
  ...
  const base = [
    ...
    // Cards used to depend entirely on the model making a SECOND call
    // (`ui_show`) after the data one, and that is the call it skips: "I did
    // have to prompt it to show cards when I was asking for info". A list
    // result now goes on screen from the bridge, by its shape, whatever tool
    // returned it — so this line's job changed from ordering the card to
    // telling the model the card is already there. The ui_show half stays for
    // everything the model composes rather than fetches, and as the fallback
    // when a result derives no card.
    "When a result is something to LOOK at — events, messages, contacts, options, search results, a table — announce it aloud first in one short sentence (for example: Here's what I found). A list result goes onto their screen by itself and the tool result tells you when it has: then just give a one- or two-sentence spoken summary and do NOT call ui_show for that same result. If the result does not say it is on screen and it is still worth seeing, call ui_show yourself. Either way the card is seen, not spoken: never read it item by item.",
    "For any calendar question, always call get_calendar and answer ONLY from its result — their calendar events are not in your briefing, memory, or messages, and answering from those is making things up. Announce aloud, then give a short spoken summary — the events go on their screen by themselves, so only call ui_show for them if the result does not say it is already shown. If get_calendar says their calendar isn't connected, tell them exactly that and that they can link Google Calendar in Settings → Connectors — never improvise a calendar answer from anything else.",
