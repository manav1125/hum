...
describe("AgentLoop — interrupted-call resume", () => {
  test("resumes when the call dies after streaming and the retry completes", async () => {
    const { provider, callCount } = scriptedProvider([
      { kind: "interrupt", thinking: "drafting the page" },
      reply("Done — the page is finished."),
    ]);

    const { events } = await runLoop(provider);

    // The interrupted call was re-issued and the turn finished cleanly.
    expect(callCount()).toBe(2);
    expect(events.filter((event) => event.type === "error")).toHaveLength(0);
  });

  test("a resumed call re-sends the same history, keeping completed tool work", async () => {
