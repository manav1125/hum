/**
 * Tests for the per-conversation temporary approval override store.
 *
 * Recovered from upstream (feature originally added in e05896063f, last
 * intact at 46d64df40d^) alongside the store itself.
 */
...
describe("conversation-approval-overrides", () => {
  afterEach(() => {
    clearAll();
  });
  ...
  describe("timed mode", () => {
    test("setTimedMode stores a timed override with default 10-minute TTL", () => {
      ...
      const timed = mode!;
      if (timed.kind === "timed") {
        expect(timed.expiresAt).toBeGreaterThanOrEqual(
          before + DEFAULT_TIMED_DURATION_MS,
        );
        expect(timed.expiresAt).toBeLessThanOrEqual(
          after + DEFAULT_TIMED_DURATION_MS,
        );
      }
    });
    ...
    test("timed mode expires after TTL elapses — expiry returns to ask, never allows", async () => {
