...
describe("FileSystemOps.readFileSafe", () => {
  ...
  test("a read that reaches the last line carries no truncation notice", () => {
    const dir = makeTempDir();
  ...
  test("paging past the end is not reported as truncation", () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, "small.txt"), "a\nb\nc");
    const ops = new FileSystemOps(sandboxPolicyFor(dir));

    const result = ops.readFileSafe({ path: "small.txt", offset: 100 });
    expect(result.ok).toBe(true);
    if (!result.ok) {
      return;
    }
