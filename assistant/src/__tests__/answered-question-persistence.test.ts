...
// ── Mocks (spread the real module; override only the seams driven here) ──────
 ...
 * them `ask_question`. Every id is registered as in-flight, so end-of-turn
 * annotation only runs once each has a result.
 */
...
describe("answered ask_question persistence", () => {
  ...
  test("stamps the answered record on the tool_use block and forwards it to clients", () => {
    ...
    void handleToolResult(state, makeDeps(), {
      type: "tool_result",
      toolUseId: "tu_q",
  ...
  test("survives the end-of-turn annotation that re-stamps the row", () => {
    ...
    void handleToolResult(state, deps, {
      type: "tool_result",
      toolUseId: "tu_q",
      content: "answered",
      isError: false,
  ...
  test("re-stamping the same record does not rewrite the row", () => {
    ...
    // it a no-op, and the end-of-turn annotation cannot re-run either (it
    // cleared `currentTurnToolUseIds` when the first result completed the
    // turn), so the row is not rewritten.
    expect(updates).toHaveLength(2);
    expect(persistedToolUse("tu_q")._answeredQuestion).toEqual(ANSWERED);
  });
