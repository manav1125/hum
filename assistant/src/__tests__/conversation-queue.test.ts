...
describe("Conversation message queue", () => {
  ...
  test("a processMessage turn keeps its turn-start trust when the slot moves before the loop opens", async () => {
    ...
    // throughout that window by paths that do not own this turn (voice
    // hydration stamp-and-restore, the voice bridge).
    //
    ...
    (
      ...
      // Also move the per-turn field, which is writable out-of-band (wake
      // flows stamp it and restore the prior value in a `finally`). Covers
      // both writers, so a fix that reads either one back at the agent loop
...
/**
 * The prompter reads the conversation-level sender, not the turn's own
 * `onEvent`. An interactive turn binds that sender to the hub and restores it
 * on the way out, but the queue drain bypasses `processMessage` — so a drained
 * turn inherited the no-op the previous turn left behind, and any
 * confirmation_request it raised was emitted into nothing and hung until the
 * permission timeout auto-denied it.
 *
 * Ported from vellum-assistant.
 */
