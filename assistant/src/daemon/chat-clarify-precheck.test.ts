...
describe("buildClarifyDirective / appendClarifyDirective", () => {
  test("directive routes to ask_question and does not answer for the user", () => {
...
describe("assessChatClarify", () => {
  ...
  test("only the FIRST question survives a multi-question reply", async () => {
    const result = await assessChatClarify(
      "book the flights",
      modelReturning(
    ...
    expect(result!.question).toBe("Which trip?");
  });
});
