 ...
 * flag is stale as boot-recovery input: it records that a turn was in flight
 ...
 *
 * Composition with the client-facing `isProcessing` signal: that flag is
 * sourced from the in-memory `Conversation` object (`findConversation(id)
 * ?.isProcessing()`), not from `processing_started_at`. After a crash there is
 * no resident object, so clients already see `isProcessing: false` and the
 * web foreground-reconcile settles the stale spinner. A resume re-hydrates the
 * conversation and calls `setProcessing(true)` (flag → true) then `(false)` on
 * completion, so `isProcessing` tracks the resumed turn correctly; a cleared or
 * capped conversation has no resident object and stays `false`.
 */
 ...
 * turn end, so a resumed turn that keeps taking the process down is left idle
 * after this many boots instead of resume-looping forever.
 */
 ...
 * not be repeated (this is also the guard against re-firing pushes/replies a
 * dying turn already dispatched).
 */
 ...
 * The wakes themselves must wait for full startup (providers, agent loop) and
 * run via {@link resumeInterruptedConversations}.
 */
...
/**
 * Resume interrupted conversations sequentially through the conversation wake
 * machinery. Sequential on purpose: a boot after a mid-turn crash can carry
 * several interrupted conversations, and running them one at a time avoids a
 * thundering herd of concurrent LLM turns right after startup.
 *
 * Each wake runs as a background turn with NO trust elevation — no
 * `trustContext` is passed, so the loop falls back to the conversation's own
 * resting trust (`trustClass: "unknown"`, side-effect tools blocked). Resuming
 * an interrupted low-trust turn therefore can never grant it guardian-class
 * capability. Failures are logged and skipped so one bad conversation doesn't
 * block the rest.
 *
