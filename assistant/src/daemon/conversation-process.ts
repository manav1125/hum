...
async function drainSingleMessage(
  ...
  // no-op the just-finished interactive turn's restore (or conversation birth)
  // left behind, and the drain bypasses `processMessage`, so without a fresh
  // bind the PermissionPrompter's confirmation_request — which reads the
  // conversation-level sender, not the turn's `onEvent` — would be emitted into
  // nothing and hang until the permission timeout auto-denied it. Same
