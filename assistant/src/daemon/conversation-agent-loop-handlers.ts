 ...
 * snapshot can render a running elapsed-time counter. The full timing + risk
 * annotation still happens in `annotatePersistedAssistantMessage` once every
 * tool in the turn completes.
 */
function recordToolStartOnPersistedMessage(
  ...
  stampToolUseBlockEarly(state, toolUseId, "tool start time", (rec) => {
    if (rec._startedAt === startedAt) return false;
