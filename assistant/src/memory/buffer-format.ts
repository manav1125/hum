...
// ---------------------------------------------------------------------------
//
// `memory/buffer.md` (and the v1 `pkb/buffer.md`) is the append-only staging
// log that `remember()` writes and consolidation drains. Its line format is
// written here and recognized here, by one matcher, so the writer and every
// reader agree by construction.
//
// This module deliberately imports nothing from the memory tiers, so every
// reader (graph tool handlers, the v2 substrate jobs, the graph-topology
// pending view) can reach it without inverting the layering.
//
...
// bullet dash, spacing inside the date), and each disagreement splits or
// merges somebody's fact.
...
export interface BufferEntryStart {
  ...
  timestamp: string;
  /**
   * Everything after the timestamp bracket, trimmed. Any trailing
   * `<!--cid:...-->` origin marker is left in place — use
   * {@link parseBufferEntryOrigin} to split it off.
   */
  text: string;
}
 ...
 * the body nested under the timestamped first line. That rule is what makes
 ...
 *
 * Backward-compatible: entries written without the marker simply omit it.
 * {@link parseBufferEntryOrigin} treats a missing marker as "no known
 * origin".
 */
const CID_MARKER_RE = /\s*<!--\s*cid:([^\s>]+)\s*-->\s*$/;

/**
 * Split a single buffer line into its origin conversation id (if the line
 * carries a `<!--cid:...-->` marker) and the line with that marker stripped
 * for display.
 *
 ...
 * two — a stored fact could forge extra entries. Indenting moves every body
 * line off column 0, so no fact content can imitate a delimiter no matter
 * what the user dictated. Blank lines stay genuinely blank rather than
 * becoming whitespace.
 *
 * Readers strip the indent: {@link matchBufferEntryStart} classifies on the
 * raw line while entry parsers trim continuation lines, so the fact text a
 * caller gets back is unchanged by this.
 *
 * When `conversationId` is supplied, a trailing `<!--cid:...-->` marker is
 * appended to the OPENING line so the origin rides the line entry parsers
 * classify on. Callers with no single source conversation (e.g. the
 * cross-conversation sweep job) omit it — those entries stay untagged and
 * parse exactly as unmarked entries do.
 *
...
export function formatRememberEntry(
  ...
  return `- [${formatBufferTimestamp(now)}] ${[`${opening}${marker}`, ...nested].join("\n")}\n`;
}
