...
export interface PageIndexEntry {
   ...
   * by recency. This stays raw mtime because reembed-delta consumers diff
   * on actual edit recency; the origin-aware recency signal lives in
   * {@link PageIndexEntry.freshAt}.
   */
  modifiedAt: number;
  /**
   ...
   * consumers read this instead of `modifiedAt`. `splitTier1` deliberately
   * keeps sorting on `modifiedAt`: consolidation-facing tier ranking reflects
   * actual edit recency.
   */
  freshAt: number | null;
}
 ...
 * and the ingest provenance warning (`ingest.ts`) both call it, so a value
 * that warns at ingest is exactly a value recency ranking ignores.
 */
