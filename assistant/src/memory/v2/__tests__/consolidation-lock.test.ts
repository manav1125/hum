/**
 * Tests for `assistant/src/memory/v2/consolidation-lock.ts`.
 *
 * Coverage matrix:
 *   - Free lock → acquired; payload records `<pid> <timestamp>`.
 *   - Advisory holder tag appended after the PID and timestamp.
 ...
 *
 * Tests use temp dirs (mkdtemp) and never touch a live workspace.
 */
import {
  existsSync,
  mkdirSync,
