/**
 * Tests for `assistant/src/memory/v2/ingest.ts`.
 *
 * Coverage matrix:
 ...
 *
 * Tests use temp workspaces (mkdtemp) and never touch a live workspace.
 * Sample content uses generic placeholders (Alice).
 */
import {
  existsSync,
  mkdirSync,
...
// assert the fan-out without initializing sqlite. The real module is spread
// so unrelated exports survive for later files in a combined run.
