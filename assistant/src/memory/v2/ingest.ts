/**
 * Memory v2 — deterministic batch concept-page ingest.
 *
 ...
 * `SKILL_SLUG_PREFIX` (`v2/skill-store.ts`) and `CLI_COMMAND_SLUG_PREFIX`
 * (`v2/cli-command-store.ts`); the literals are duplicated here because
 * importing those modules would pull the embedding-backend chain into every
 * ingest caller (the page index lazy-imports them for the same reason). The
 * page index drops concept pages whose slugs collide with synthetic entries,
 * so ingesting one would persist an unreachable file.
 */
 ...
 * maintenance (a no-op job on installs where the v3 shadow is not live).
 */
 ...
 *   2. Content parse via `parsePageContentStrict`. A schema failure or a
 *      degraded parse (unterminated frontmatter fence) yields `invalid` with
 *      the parse error string: staging content can still be fixed by its
 *      author, so validation is per-page and loud, never a silent salvage
 *      (the lenient salvage path stays reserved for pages already on disk).
 ...
 *      uses, so a warned date is exactly one the fresh signal ignores), and a
...
export async function ingestPages(
  ...
  for (const input of pages) {
    ...
    try {
      const parsed = parsePageContentStrict(input.slug, input.content);
      if (parsed.parseWarning !== undefined) {
        results.push(invalid(parsed.parseWarning));
        continue;
      }
      page = parsed;
    } catch (err) {
