 ...
 *
 * In this fork the active backend is memory v2 + the v3-shadow edge layer,
 * whose knowledge corpus is already an Obsidian-style graph: markdown concept
 * pages (identified by slug, under `memory/concepts/`) linked by authored
 * `links:` frontmatter / inline `[[wikilinks]]` (the STATIC link graph). This
 * module reads exactly the same sources the retrieval lanes build from
 * (`getPageIndex` + `buildEdgeGraph`) and maps them into the backend-agnostic
 * contract — no embeddings, no LLM calls.
 *
 * GATE NOTE (deliberate divergence from upstream): upstream gates this on
 * `isV3TierActive`. We run v2 + v3-shadow over the same `memory/concepts/*.md`
 * tree, so gating on a v3 tier would report `supported: false` everywhere and
 * ship the feature dead. Instead the graph is supported whenever memory v2 is
 * live (`memory.enabled !== false && memory.v2.enabled`), and the backend
 * names itself `memory-v2`.
 *
 ...
 * / DB I/O. When a memory-backend plugin seam lands, `getMemoryGraph` becomes
...
} from "../../plugins/defaults/memory-v3-shadow/capabilities.js";
import { buildEdgeGraph } from "../../plugins/defaults/memory-v3-shadow/edge.js";
import type { Slug } from "../../plugins/defaults/memory-v3-shadow/types.js";
...
/** Backend identifier stamped onto graphs produced from memory v2. */
const BACKEND_MEMORY_V2 = "memory-v2";
/**
...
/**
 * Whether the concept graph can be built for this config: the v2 concept-page
 * substrate is what the graph reads, so it is supported exactly when memory v2
 * is live. See the module GATE NOTE.
 */
export function isGraphSupported(config: AssistantConfig): boolean {
...
/**
 * Pure mapping from the article + edge structures to the canonical graph.
 *
 ...
 * unsupported, empty graph when memory v2 is not the live backend, so callers
...
export async function getMemoryGraph(
  ...
  // Raw (frontmatter + body) page reader, matching the retrieval lane build. A
  // read that rejects drops that article's authored/wikilink edges but keeps
  // its numeric fallbacks.
