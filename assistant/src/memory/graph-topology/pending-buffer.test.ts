...
describe("parseBufferEntries", () => {
  ...
  test("strips the <!--cid:...--> origin marker our writer appends", () => {
    const entries = parseBufferEntries(
      formatRememberEntry("Alice prefers VS Code", new Date(), "conv-123"),
    );
  ...
  // TODO(buffer-format): upstream also guarantees that a fact whose BODY is an
  // entry-shaped dated bullet at column 0 stays one node, because its writer
  // nests continuation lines. Our `formatRememberEntry` writes body lines
  // verbatim, so that guarantee can't hold until the shared
  // `assistant/src/memory/buffer-format.ts` module lands (see the module TODO
  // in pending-buffer.ts). Port upstream's round-trip test then.
