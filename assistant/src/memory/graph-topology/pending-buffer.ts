 ...
 * consolidation nudge.
 *
 * TODO(buffer-format): upstream centralizes the buffer entry grammar in
 * `assistant/src/memory/buffer-format.ts` (`formatRememberEntry` +
 * `matchBufferEntryStart`), which also nests entry-shaped body lines so a
 * multiline fact can't be split into several entries. That module doesn't
 * exist in this fork yet (it's being created separately for the S3 fencing
 * work); until it lands, the local `matchBufferEntryStart` below mirrors the
 * exact shape `formatRememberEntry` in `../graph/tool-handlers.ts` writes and
 * `extractBufferEntryTimestamp` in `../v2/consolidation-job.ts` parses.
 * Converge on the shared module when it exists.
 */
...
/**
 * A timestamped entry start: `- [Mon D, h:mm AM/PM] text`. Matched against the
 * untrimmed line — an indented entry-shaped bullet is body text inside the
 * fact above it, not a fact of its own. The bracket contents must match the
 * `formatBufferTimestamp` shape exactly so markdown checklists (`- [ ] …`)
 * and wikilink bullets (`- [[…]]`) never count as entry starts (mirrors
 * `extractBufferEntryTimestamp` in `../v2/consolidation-job.ts`).
 */
const ENTRY_START_REGEX =
  /^-\s*\[([A-Z][a-z]{2} \d{1,2}, \d{1,2}:\d{2} [AP]M)\]\s?(.*)$/;

/** Local stand-in for the shared buffer grammar (see the module TODO). */
function matchBufferEntryStart(
  line: string,
): { timestamp: string; text: string } | null {
  const match = ENTRY_START_REGEX.exec(line);
  if (!match) {
    return null;
  }
  // Strip the trailing `<!--cid:...-->` origin marker our writer appends —
  // it's provenance metadata, not part of the fact text.
  const { display } = parseBufferEntryOrigin(match[2]!);
  return { timestamp: match[1]!, text: display.trim() };
}
...
export interface PendingBufferEntry {
  ...
  id: string;
  /** The entry's fact text, timestamp and origin marker stripped, wikilink
   * markup intact. */
  text: string;
...
export function parseBufferEntries(content: string): PendingBufferEntry[] {
  ...
  for (const rawLine of content.split("\n")) {
    ...
    if (current !== null) {
      current.push(parseBufferEntryOrigin(line.trim()).display.trim());
      continue;
    }
    const plainBullet = PLAIN_BULLET_REGEX.exec(line.trim());
    if (plainBullet) {
      texts.push(parseBufferEntryOrigin(plainBullet[1]!.trim()).display.trim());
    }
  }
  flush();
