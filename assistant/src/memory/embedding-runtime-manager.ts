...
import { arch, platform, setPriority } from "node:os";
...
/**
 * Spawn-time ONNX intra-op thread cap for the embed and rerank workers, passed
 * to the worker script as a positional argument.
 */
export function onnxThreadArg(): string {
  return String(getEmbedOnnxThreads());
}

/**
 * Deprioritise a freshly spawned ML worker so the daemon's event loop wins the
 * CPU whenever both are runnable. Best-effort: renicing is unavailable on some
 * platforms and forbidden for negative values without privileges, and a worker
 * that keeps its default priority is still a working worker — never let this
 * fail a spawn.
 */
export function deprioritizeWorker(pid: number | undefined): void {
  const niceness = getEmbedWorkerNiceness();
  if (niceness === 0 || pid === undefined) return;
  try {
    setPriority(pid, niceness);
  } catch (err) {
    log.debug({ err, pid, niceness }, "Could not renice ML worker process");
  }
}
 ...
 */
const RUNTIME_VERSION = `ort-${ONNXRUNTIME_NODE_VERSION}_hf-${TRANSFORMERS_VERSION}_jinja-${JINJA_VERSION}_workers-v4`;
...
export function generateWorkerScript(): string {
  ...
  //
  // The ONNX intra-op thread count arrives as argv[4] so the cap is a
  // spawn-time decision rather than being baked into the cached script.
  // Without it ONNX Runtime opens one intra-op thread per core and a single
  // background batch occupies every CPU on a small box, starving the daemon's
  // event loop (see `getEmbedOnnxThreads`).
  return `\
...
const threads = Math.max(1, Number(process.argv[4]) || 1);
...
export function generateRerankWorkerScript(): string {
  ...
  //
  // argv[5] carries the ONNX intra-op thread cap, for the same event-loop
  // starvation reason documented on the embed worker above.
  return `\
...
const threads = Math.max(1, Number(process.argv[5]) || 1);
if (cacheDir && env) env.cacheDir = cacheDir;
...
try {
  ...
  session = await AutoModelForSequenceClassification.from_pretrained(model, {
    dtype,
    session_options: {
      intraOpNumThreads: threads,
      interOpNumThreads: 1,
