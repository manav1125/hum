...
| `HQ_DRIVER` | `render` to use the Render driver (default: mock) |
...
- **Render driver is untested against the live API** (unit-tested via mock
  fetch shape only). It creates an image-backed web service mirroring
  render.yaml's `cue-app`; the blueprint itself can't be instantiated via
  API. Validate against a scratch Render workspace before first real use.
- **No per-instance Qdrant**: render.yaml provisions a private `cue-qdrant`
  pserv; the driver currently only creates the web service. Pass a shared
  `QDRANT_URL` via `providerEnv`, or extend the driver with a second create.
